...
export class ConversationalInterviewClient {
  ...
  private userFirstName: string // SIMPLIFIED: Only store user's first name
  ...
  private constructor(
    ...
    questions: any,
    jobContext: any,
    userFirstName: string, // SIMPLIFIED: Only accept user's first name
    ...
    // SIMPLIFIED: Just use the provided userFirstName directly
    this.userFirstName = userFirstName || "the candidate"

    console.log(`🎭 Selected interviewer: ${this.interviewerName}`)
    console.log(`👤 Using candidate name: ${this.userFirstName}`)
  }
  ...
  static async create(
    ...
    const client = new ConversationalInterviewClient(
      ...
      questions,
      jobContext,
      userFirstName, // SIMPLIFIED: Pass userFirstName directly
      config,
      callbacks,
    )
  ...
  private async generateQuestionAudio(
    ...
    try {
      ...
      if (abortController?.signal.aborted) {
        throw new Error("Request aborted before audio generation")
      }

      // Use the server-side API route for audio generation
      const response = await fetch("/api/interview/generate-live-audio", {
        method: "POST",
        headers: {
      ...
      // Check if request was aborted after generation
      if (abortController?.signal.aborted) {
        throw new Error("Request aborted after audio generation")
      }

      if (data.success && data.audioData) {
        console.log(`✅ Audio generated successfully using Gemini Live API (${data.audioLength} characters)`)
      ...
      if (abortController?.signal.aborted || error.message.includes("aborted")) {
        console.log(`🚫 Audio generation aborted: ${questionText.substring(0, 50)}...`)
