...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Extract job data from the associations
    const associatedJobs = jobAssociations.map((association) => association.jobs).filter((job) => job !== null)

    return NextResponse.json({ success: true, jobs: associatedJobs })
  } catch (error) {
