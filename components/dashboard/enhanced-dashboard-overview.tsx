    ...
    const fetchUserName = async () => {
      try {
        ...
        if (!contentType || !contentType.includes("application/json")) {
          console.error("User profile API returned non-JSON response:", contentType)
          return
        }
        ...
        if (data.user) {
          // Try first_name first, then fall back to extracting from full_name or name
          const firstName =
        ...
        // Don't set any name if there's an error - just continue without personalization
      }
    }
