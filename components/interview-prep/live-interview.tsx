...
export function LiveInterview({
  ...
  useEffect(() => {
    console.log("🔍 LiveInterview state:", {
      ...
      isPreloaded,
      canStartInterview,
      userFirstName, // SIMPLIFIED: Only log userFirstName
    })
  ...
  const startConversationalInterview = async () => {
    try {
      ...
      // SIMPLIFIED: Create interview client with only userFirstName
      const client = await ConversationalInterviewClient.create(
        job.id,
        resume?.id,
        questions,
        jobContext,
        userFirstName, // SIMPLIFIED: Pass only userFirstName
        {
          voice: selectedVoice,
